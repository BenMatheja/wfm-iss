...
public class EvaluateDesignController implements Serializable{
	...
	public Design getDesignEntity() {
		if (designEntity == null) {
		      // Load the entity from the database if not already cached
		      LOGGER.log(Level.INFO, "This is designId from businessProcess: " + businessProcess.getVariable("designId")); 
			  LOGGER.log(Level.INFO, "This is getDesign from the Service invoked with it " + designService.getDesign((Long) businessProcess.getVariable("designId")));
		      designEntity = designService.getDesign((Long) businessProcess.getVariable("designId"));
		} else {
